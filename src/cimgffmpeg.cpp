...
void vfinfo_close(VFInfo* vfinfo) {
  if (vfinfo->pFormatCtx != nullptr) {
    avcodec_close(vfinfo->pCodecCtx);
    vfinfo->pCodecCtx = nullptr;
    avformat_close_input(&vfinfo->pFormatCtx);
    vfinfo->pFormatCtx = nullptr;
    vfinfo->width = -1;
    vfinfo->height = -1;
  }
}
...
int ReadFrames(VFInfo* st_info, cimg_library::CImgList<uint8_t>* pFrameList, long low_index, long hi_index) {
  ...
  if (st_info->pFormatCtx == nullptr) {
    ...
    // Open video file
    if (avformat_open_input(&st_info->pFormatCtx, st_info->filename, nullptr, nullptr) != 0)
      return -1; // Couldn't open file

    // Retrieve stream information
    if (avformat_find_stream_info(st_info->pFormatCtx, nullptr) < 0)
      return -1; // Couldn't find stream information

    //dump_format(pFormatCtx,0,nullptr,0);//debugging function to print infomation about format
    ...
    if (st_info->pCodecCtx == nullptr) {
      return -1;
    }

    // Find the decoder
    st_info->pCodec = avcodec_find_decoder(st_info->pCodecCtx->codec_id);
    if (st_info->pCodec == nullptr) {
      return -1; // Codec not found
    }
    // Open codec
    if (avcodec_open2(st_info->pCodecCtx, st_info->pCodec, nullptr) < 0)
  ...
  avpicture_fill(reinterpret_cast<AVPicture *>(pConvertedFrame), buffer, ffmpeg_pixfmt, st_info->width, st_info->height);
...
int GetNumberStreams(const char* file) {
  ...
  av_register_all();
  // Open video file
  if (avformat_open_input(&pFormatCtx, file, nullptr, nullptr))
    return -1 ; // Couldn't open file
...
int64_t GetNumberVideoFrames(const char* file) {
  ...
  if (nb_frames > 0) {
    ...
    avformat_close_input(&pFormatCtx);
    return nb_frames;
  }// frames must be counted

  // AVPacket packet;
  nb_frames = static_cast<long>(av_index_search_timestamp(str, str->duration, AVSEEK_FLAG_ANY | AVSEEK_FLAG_BACKWARD));
...
float fps(const char* filename) {
  ...
  // Open video file
  if (avformat_open_input(&pFormatCtx, filename, nullptr, nullptr))
    return -1; // Couldn't open file
